/**
 * HTML template generator for LLM debug info windows.
 */
...
export function createDebugInfoHTML(metadata: any): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    ...
    <div class="section response-section">
      ...
      <div class="response-content">${escapeHtml(metadata.response?.substring(0, 5000) || '')}${(metadata.response?.length || 0) > 5000 ? '\n\n... (truncated, see full metadata below)' : ''}</div>
    </div>
